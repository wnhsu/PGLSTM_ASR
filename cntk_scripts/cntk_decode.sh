...
scoring_opts="--min-lmwt 5 --max-lmwt 20"
skip_scoring=false
...
inputCounts="$sdata/JOB/cntk_dev.counts"
inputfeats="$sdata/JOB/cntkOutput.scp"
...
if [ ! -f $sdata/JOB/sls.scp ]; then
    ...
    $cmd JOB=1:$nj $dir/log/make_input.JOB.log \
        echo scp:$sdata/JOB/feats.scp \> $inputfeats
fi

# Run the decoding in the queue
if [ $stage -le 0 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    $cnstring inputCounts=$inputCounts inputFeats=$inputfeats \| \
